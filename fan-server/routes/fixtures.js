...
router.get('/debug/:seasonId', async (req, res) => {
  try {
    ...
    const debug = {
      ...
      sampleRegularSeasonFixtures: fixtures.regularFixtures.slice(0, 5).map(f => ({
        round: f.round,
        date: f.date,
    ...
    if (round) {
      ...
      const roundFixtures = fixtures.allFixtures.filter(f => f.round === roundNumber);
      ...
      debug.specificRound = {
        ...
        fixtures: roundFixtures.map(f => ({
          ...
          teams: `${f.homeTeam} vs ${f.awayTeam}`,
          season: f.season,
          beforeCutoff: new Date(f.date) <= new Date(fixtures.regularSeasonEndDate)
        }))
      };
    }
